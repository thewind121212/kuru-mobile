...
**Before doing any UI work in this repo (MANDATORY for implementer subagents):** read `.claude/skills/mobile-design/SKILL.md`. Documents the canonical **screen archetypes** (list / detail / form) with the exact reference files to mirror, the **pastel icon-tint vocabulary**, naming conventions, analyzer trip-wires (the ones implementer subagents kept hitting), test patterns (when not to use `pumpAndSettle`), and theme tokens. When dispatching a subagent for any design / restyle / new-screen task, paste the explicit instruction `Read .claude/skills/mobile-design/SKILL.md before writing code; mirror the archetype reference file the skill points to.` in the prompt. The Claude `Skill` tool can also be invoked with `mobile-design` to load it on demand.
